...
prior-auth-copilot/
  ...
  src/pa_copilot/
    ...
    rag/           corpus.py  index.py  tool.py
    mcp_server/    __main__.py  server.py  data/{benefits,providers,criteria}.json
...
|---|---|---|---|
| tool | `benefit_lookup` | `(member_id, service_code)` -> `covered`, `requires_pa`, `network_status`, `plan_id` | `data/benefits.json` |
| tool | `criteria_check` | `(service_code, diagnosis_codes)` -> applicable `policy_id` + structured requirement checklist + `status` (`met`/`not_met`/`indeterminate` best-effort) | `data/criteria.json` |
| tool | `provider_lookup` | `(npi)` -> `name`, `specialty`, `network_status` | `data/providers.json` |
| resource | `pa://criteria/{policy_id}` and `pa://criteria/index` | full structured coverage-policy document (required conditions, exclusions, evidence requirements) / index of all policy ids | `data/criteria.json` |
...
- **Corpus** — `data/synthetic/clinical_guidance/`: ~12 synthetic payer clinical-guidance
  documents (narrative: indications, step-therapy, conservative-care duration, exclusions)
  covering the sample services. Generated by `rag/corpus.py`; fully synthetic; committed.
- **Index** — `rag/index.py`: in-process Chroma (`./.pa_chroma/`), bge-small local
  embeddings, clause-level chunks. Built by `pac ingest`.
