...
|---|---|---|---|---|---|
| **NFR-01** | No secrets or API keys committed; env-var config with a committed `.env.example` | `.env.example` committed; `.env` in `.gitignore`; all keys read via `os.environ` in `config.py` | `tests/test_nfr01_no_secrets.py` — scans the tree for key-shaped strings and for a committed `.env` | scaffold, PR7 | partial (`.env.example` done) |
...
| **NFR-04** | Structured JSON logs / traces of agent runs are committed as evidence | `tracing.py` writes one JSON trace per run to `traces/` | `tests/test_nfr04_trace_schema.py` — validates every `traces/*.json` against the trace schema | PR1, PR7 | pending |
| **NFR-05** | All data synthetic; any PII synthetic and never written to logs in plaintext | synthetic generators in `data/synthetic/`; `tracing.py` redacts `member_id` and configured PII fields before write | `tests/test_nfr05_redaction.py` — asserts `member_id` and synthetic names absent from every `traces/*.json` | PR1 | pending |
